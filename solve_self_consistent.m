                        ...
                        if isempty(DG.rho) || isempty(DG.n)
                            error('DG:Full2D:SolveSkipped', ...
                                ['Full 2D DG/FV operator was built, but no transport solve was run. ', ...
                                 'Current Full-2D status is "%s". For a self-consistent run, set ', ...
                                 'mat.dg.params.full2D_solve = true and choose a matrix-free solver, ', ...
                                 'or reduce the Full-2D grid for a small assembled test.'], DG.status);
                        end
