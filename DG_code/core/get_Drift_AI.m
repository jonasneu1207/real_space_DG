function G_glob = get_Drift(p,U_IV)
    if (p.doIV == true)
        p.U = U_IV;
    ...
    % Better size estimation for pre-allocation
    estimated_nonzeros = ceil(0.1 * Ny * Ny * p.N_chi * p.N_K_chi * p.N_K_chi); % Assume ~10% density
    index_zl = zeros(estimated_nonzeros, 1);
    index_sp = zeros(estimated_nonzeros, 1);
    werte_Drift = zeros(estimated_nonzeros, 1);
    current_idx = 1;

    % Pre-compute some indices to avoid repeated calculations
    zl_base = (0:p.N_K_chi*p.N_chi-1)';

    for j = 1:Ny
        zl = zl_base + (j-1)*(p.N_K_chi*p.N_chi) + 1;
        
        for m = 1:Ny
            sp = (m-1)*(p.N_K_chi*p.N_chi) + (1:p.N_K_chi*p.N_chi);
            
            for d = 1:p.N_chi
                G_block = G((d-1)*p.Nqx+1:d*p.Nqx,j,m);
                
                if norm(G_block) > tol
                    xBlock = (d-1)*p.N_K_chi+1 : d*p.N_K_chi;
                    block_size = length(xBlock)^2;
                    
                    % Ensure enough space is available
                    if current_idx + block_size > length(index_zl)
                        % Grow arrays by 50% if needed
                        new_size = ceil(length(index_zl) * 1.5);
                        index_zl(end+1:new_size) = 0;
                        index_sp(end+1:new_size) = 0;
                        werte_Drift(end+1:new_size) = 0;
                    end
                    
                    % Direct indexing instead of meshgrid
                    ...
                    idx_range = current_idx:current_idx+block_size-1;
                    index_zl(idx_range) = zl_inner(:);
                    index_sp(idx_range) = sp_inner(:);
                    werte_Drift(idx_range) = Drift(:);
    ...
    % Trim excess zeros
    index_zl = index_zl(1:current_idx-1);
    index_sp = index_sp(1:current_idx-1);
    werte_Drift = werte_Drift(1:current_idx-1);
