...
% index_zl = zeros(p.dim/p.N_chi,1);
% index_sp = index_zl;
...
%% Wladis version
zl = zeros(p.N_K_chi*p.N_chi,Ny);
sp=zl';
for j = 1 : Ny
    zl(:,j) = ((j-1)*(p.N_K_chi*p.N_chi)+1 : j*(p.N_K_chi*p.N_chi))';
    sp(j,:) = (j-1)*(p.N_K_chi*p.N_chi)+1 : j*(p.N_K_chi*p.N_chi);
end
...
% for d = 1 : p.N_chi
G = G.*w_all;      % Gauß-Lobatto Quadratur
for j = 1:Ny
    for m = 1:Ny
        if (norm(G(:,j,m)) > tol)          % nummerische Nullen ausgrenzen
            % xBlock = (d-1)*p.N_K_chi+1 : d*p.N_K_chi;
            % zl_inner = zl(xBlock)*ones(1, p.N_K_chi);
            % sp_inner = ones(p.N_K_chi, 1)*(sp(xBlock));
            W_tilde = W_all.*G(:,j,m);
            G_glob((j-1)*p.N_K_chi*p.N_chi+1:j*p.N_K_chi*p.N_chi,(m-1)*p.N_K_chi*p.N_chi+1:m*p.N_K_chi*p.N_chi) = p.Q_drift*invM_all.*(W_tilde.'*W_all);
            % werte_Drift(index) = Drift(:);
            % index_zl(index) = zl_inner;
            % index_sp(index) = sp_inner;
            % index = index + p.N_K_chi*p.N_K_chi;
