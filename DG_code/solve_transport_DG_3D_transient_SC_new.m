function DG = solve_transport_DG_3D_transient_SC_new(mat, INIT)
    ...
    Vsub_INIT = INIT.Vsub;
    A_INIT    = INIT.A;
    rhs_INIT = cell(1,n_of_modes);
    for IM = 1:n_of_modes
     rhs_INIT{IM}  = mat.dg.params.MPC*INIT.rhs{IM};
    end
    rho_INIT  = INIT.rho;
    p         = INIT.p;

    Vbi = squeeze(INIT.V_total);
  
    %% Get subband energies, system matrix and boundary conditions for gate voltage after switching 
       
    % Vg_temp = mat.Vg;
    % mat.Vg  = Vg_temp(1);
    % DG = solve_self_consistent_3D(mat, 'DG');
    % 
    % mat.Vg = Vg_temp;
    % 
    % % 
    ...
    for IM = 1:n_of_modes
        if mat.dg.params.permute_shell
            rho_pre(IM,:,:) = ((reshape(squeeze(rho_INIT{IM}),p.Nk, p.N_chi*p.N_K_chi)).'*p.phi.');
        else
    ...
    Vsub_mod     = zeros(n_of_modes, p.N_chi*p.N_K_chi, mat.Ny, mat.Nz);
    Vsub_mod_pre = zeros(n_of_modes, p.N_chi*p.N_K_chi, mat.Ny, mat.Nz);
    m_mod        = zeros(n_of_modes, p.N_chi*p.N_K_chi,1);
    n_pre        = zeros(n_of_modes, p.N_chi*p.N_K_chi, mat.Ny, mat.Nz);
    IN_max  = size(n_pre,2);
    
    for IM=1:n_of_modes            
            %% Valley and Mode specific values
            %EM = squeeze(Esub(IV, :, IM));                           % Valley and Mode specific Subband energy
            m = squeeze(mat.me_x(1, :, ceil(mat.Ny/2), ceil(mat.Nz/2)))*m0;
   
            %% Mapping of Vsub, Esub onto DG_chi coordinates            
            for Nchi=1:p.N_chi
                V_temp = reshape(Vsub_INIT(1,(Nchi-1)*(p.N_K_chi-1)+1:(Nchi)*(p.N_K_chi-1)+1, IM,:),p.N_K_chi,mat.Ny, mat.Nz);     
                Vsub_mod(IM, (Nchi-1)*p.N_K_chi+1:Nchi*p.N_K_chi,:,:) = V_temp;

                V_temp_pre = reshape(Vsub_INIT(1,(Nchi-1)*(p.N_K_chi-1)+1:(Nchi)*(p.N_K_chi-1)+1, IM,:),p.N_K_chi,mat.Ny, mat.Nz);     
            ...
            %rho_temp = (reshape(rho_INIT{IM}, p.N_chi*p.N_K_chi, p.Nk)*p.phi.').';
            rho_temp = squeeze(rho_pre(IM,:,:));
            %rho_u_temp = (reshape(rho_temp, p.N_chi*p.N_K_chi, p.Nk)*p.phi.').';
            n_temp = (real(rho_temp(:, p.N_xi/2+1)+ real(rho_temp(:, p.N_xi/2)))/2);
            for IN=1:IN_max
                     n_pre(IM,IN,:,:) = (squeeze(n_temp(IN,1)).*squeeze(abs(Vsub_mod_pre(IM,IN,:,:))).^2/mat.dy/1E-9/mat.dz/1E-9);
            end
    end
    ...
    n_over_t = zeros(p.N_chi*p.N_K_chi, mat.Ny, mat.Nz, mat.Nt);
    j_over_t = zeros(p.N_chi*p.N_K_chi, mat.Nt);
    
    n_over_t_mode = zeros(n_of_modes, p.N_chi*p.N_K_chi, mat.Ny, mat.Nz, mat.Nt);
    ...
    for tstep = 1 : Nt
        ...
        if (mod((tstep-1),20) == 0 || (tstep<100))
            Vref = mat.V; mat.V = Vref+Vbi;
            [EM, VM] = solve_subbands_3D(mat);
            mat.V = Vref;
        end

        [Ef0_L, Ef0_R] = solve_fermi_3D(EM, mat);
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
        ...
        for IM = 1:n_of_modes
            ...
            if mat.dg.params.permute_shell
                result_matrix = (reshape(squeeze(rho_INIT{IM}), p.Nk, p.N_chi*p.N_K_chi)).'*p.phi.';
            else
            ...
            if mod(p.N_xi,2)==0
                n_DG = (real(result_matrix(:,p.N_xi/2+1)+ real(result_matrix(:,p.N_xi/2)))/2);
                for IN=1:IN_max
                     n_over_t_mode(IM,IN,:,:, tstep) = (squeeze(n_DG(IN,1)).*squeeze(abs(Vsub_mod(IM,IN,:,:))).^2/mat.dy/1E-9/mat.dz/1E-9);
                end
            ...
            elseif mod(p.N_xi,2)==1
                ...
                n_DG = real(result_matrix(:,p.N_xi/2));
                for IN=1:IN_max
                     n_over_t_mode(IM,IN,:,:, tstep) = (squeeze(n_DG(IN,1)).*squeeze(abs(Vsub_mod(IM,IN,:,:))).^2/mat.dy/1E-9/mat.dz/1E-9);
                end
        ...
        if mod((tstep-1),100) == 0
            figure(fid)
            title(['t=', num2str(mat.t(tstep)), 'fs']);
            plot(p.chi_Koord,real(squeeze(n_t_pre(:,ceil(mat.Ny/2),ceil(mat.Nz/2)))), 'r');
            hold on
            plot(p.chi_Koord,real(squeeze(n_over_t(:,ceil(mat.Ny/2),ceil(mat.Nz/2),tstep))), 'b');
        ...
        if (mod((tstep),20) == 0 || (tstep<100))
            for IY = 1 : mat.Ny
                for IZ = 1 : mat.Nz
                    n_sp(:,IY,IZ) = Mapping*squeeze(n_over_t(:,IY,IZ,tstep));
                end
            end
    
            Vbi_IN = squeeze(INIT.V_total);
            Vbi = solve_Poisson3D_transient(mat, Vbi_IN, INIT.Ef0, tstep, n_sp);
        end
